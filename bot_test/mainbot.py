...
async def start(message: types.Message):
    ...
    with open('photo_1.jpg', 'rb') as img:
        new_message = await message.answer_photo(img, f'Добро пожаловать, {message.from_user.full_name}!👋\n' + texsts.start_text, parse_mode=types.ParseMode.HTML, reply_markup=start_kb)
...
async def consultation(call):
    ...
    with open('photo_consultation.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, texsts.consultation_text, reply_markup=consultation_kb)
...
async def free_courses(call):
    ...
    with open('photo_4.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, texsts.free_courses_text, parse_mode=types.ParseMode.HTML, reply_markup=free_courses_kb)
        last_message_id = new_message.message_id  # Сохраняем ID нового сообщения
    await call.answer()
...
async def about_instagram(call):
    ...
    with open('photo_3.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, texsts.about_instagram_text, reply_markup=about_instagram_kb)
        last_message_id = new_message.message_id  # Сохраняем ID нового сообщения
    await call.answer()
...
async def story_notebook(call):
    ...
    if last_message_id:
        try:
            ...
            logging.error(f"Ошибка при удалении сообщения: {e}")
    with open('photo_3.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, texsts.story_notebook_text, reply_markup=story_notebook_kb)
        last_message_id = new_message.message_id  # Сохраняем ID нового сообщения
...
async def inst_marathon(call):
    ...
    with open('photo_2.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, texsts.instagram_marathon_text, parse_mode=types.ParseMode.HTML, reply_markup=instagram_marathon_kb)
        last_message_id = new_message.message_id  # Сохраняем ID нового сообщения
    await call.answer()
