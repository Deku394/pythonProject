...
async def start(message: types.Message):
    ...
    with open('photo_1.jpg', 'rb') as img:
        new_message = await message.answer_photo(img, f'{message.from_user.username}, привет 🎀\n' + texst.start_text, parse_mode=types.ParseMode.HTML, reply_markup=start_kb)
...
async def begin(call):
    ...
    with open('photo_2.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, texst.congratulations_text, parse_mode=types.ParseMode.HTML, reply_markup=begin_kb)
        last_message_id = new_message.message_id  # Сохраняем ID нового сообщения
    await call.answer()
...
async def back_list_1(call):
    ...
    with open('photo_1.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, f'{call.from_user.username}, привет 🎀\n' + texst.start_text,
                                                 parse_mode=types.ParseMode.HTML, reply_markup=start_kb)
        last_message_id = new_message.message_id

@dp.callback_query_handler(text='watch_lesson')
async def begin(call):
    global last_message_id  # Объявляем переменную как глобальную
    ...
    with open('photo_3.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, texst.lesson_1_text, parse_mode=types.ParseMode.HTML, reply_markup=watch_lesson_kb)
        last_message_id = new_message.message_id  # Сохраняем ID нового сообщения
    await call.answer()
...
async def back_list_2(call):
    ...
    with open('photo_2.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, texst.congratulations_text, parse_mode=types.ParseMode.HTML,
...
async def next_block_2(call):
    ...
    with open('photo_6.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, texst.wow_text, parse_mode=types.ParseMode.HTML, reply_markup=next_block_2_kb)
        last_message_id = new_message.message_id  # Сохраняем ID нового сообщения
    await call.answer()
...
@dp.callback_query_handler(text='watch_lesson_3')
async def get_gift(call):
    ...
    with open('photo_7.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, texst.lesson_3_text, parse_mode=types.ParseMode.HTML, reply_markup=watch_lesson_3_kb)
        last_message_id = new_message.message_id  # Сохраняем ID нового сообщения
    await call.answer()
...
async def back_list_6(call):
    ...
    with open('photo_6.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, texst.wow_text, parse_mode=types.ParseMode.HTML,
                                                      reply_markup=next_block_2_kb)
        last_message_id = new_message.message_id  # Сохраняем ID нового сообщения
    await call.answer()

@dp.callback_query_handler(text='next_block_3')
async def get_gift(call):
    global last_message_id  # Объявляем переменную как глобальную
    ...
    with open('photo_8.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, f'<b>{call.from_user.username},</b>' + texst.finish_text, parse_mode=types.ParseMode.HTML, reply_markup=next_block_3_kb)
        last_message_id = new_message.message_id  # Сохраняем ID нового сообщения
    await call.answer()

@dp.callback_query_handler(text='back_list_7')
async def back_list_7(call):
    global last_message_id  # Объявляем переменную как глобальную
    ...
    with open('photo_7.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, texst.lesson_3_text, parse_mode=types.ParseMode.HTML,
                                                      reply_markup=watch_lesson_3_kb)
        last_message_id = new_message.message_id  # Сохраняем ID нового сообщения
    await call.answer()
...
@dp.callback_query_handler(text='back_list_8')
async def back_list_8(call):
    global last_message_id  # Объявляем переменную как глобальную
    ...
    with open('photo_8.jpg', 'rb') as img:
        new_message = await call.message.answer_photo(img, f'<b>{call.from_user.username},</b>' + texst.finish_text,
                                                      parse_mode=types.ParseMode.HTML, reply_markup=next_block_3_kb)
        last_message_id = new_message.message_id  # Сохраняем ID нового сообщения
    await call.answer()
