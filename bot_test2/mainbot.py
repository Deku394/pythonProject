...
async def start(message: types.Message):
    ...
    with open('photo_1.jpg', 'rb') as img:
        await message.answer_photo(img, f'<b>Добро пожаловать, {message.from_user.username}!👋</b> ' + texst.start_text, parse_mode=types.ParseMode.HTML, reply_markup=start_kb)
...
async def begin(call):
    with open('photo_2.jpg', 'rb') as img:
        await call.message.answer_photo(img, texst.lesson_1_text, parse_mode=types.ParseMode.HTML, reply_markup=begin_kb)
        await call.answer()
...
async def next_block_2(call):
    with open('photo_5.jpg', 'rb') as img:
        await call.message.answer_photo(img, texst.lesson_3_text, parse_mode=types.ParseMode.HTML, reply_markup=next_block_2_kb)
        await call.answer()

@dp.callback_query_handler(text='get_gift')
async def get_gift(call):
    with open('photo_6.jpg', 'rb') as img:
        await call.message.answer_photo(img, f'<b>{call.from_user.username},</b>' + texst.finish_text, parse_mode=types.ParseMode.HTML, reply_markup=get_gift_kb)
        await call.answer()
